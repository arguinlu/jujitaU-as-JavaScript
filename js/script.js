...
function refreshCartHtml() {
  for (let index = 0; index < carrito.length; index++) {
    ...
    if (item != null) {
      ...
      checkOut.innerHTML = `  <div><p><strong>${item.name} - </strong> Hay ${
        item.onCart
      } en el carrito, por un valor total de $${
...
function footerRefresh(cardIndex) {
  if (productos[cardIndex].stock < 1) {
    cardsStock[
      cardIndex
    ].innerHTML = `<p>$${productos[cardIndex].price} - <strong class="agotado">AGOTADO</strong></p>`;
  }
}
function htmlDeploy() {
  // CARGAR ARRAYS
...
function refreshCart(indexSelected) {
  ...
  var checkOut = document.createElement("label");
  checkOut.innerHTML = `  <div><p><strong>${idSelected.name} - </strong> Hay ${
    idSelected.onCart
  } en el carrito, por un valor total de $${
    idSelected.price * idSelected.onCart
  }</p></div>`;
...
function comprar(indexSelected) {
  ...
  if (idSelected.stock > 0) {
    ...
    guardarLocal("carritoValor", cart);
    idSelected.stock -= 1;
    idSelected.onCart += 1;
...
finish.onclick = () => {
  cartCheck = localStorage.getItem("carritoValor");
  console.log(cartCheck);
  Swal.fire({
    title: "Felicidades!",
    text: "Tu compra de " + cartCheck + "$ ha sido realizada correctamente.",
    icon: "success",
    confirmButtonText: "Cool",
  });
  carrito = [];
